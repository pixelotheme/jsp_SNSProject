...
public class BoardDAO {
	...
	public List<BoardVO> list(PageObject pageObject) throws Exception {
		...
		try {
			...
			//    1) 원본 데이터 가져오기
			String sql = " select b.no, b.title, b.content, b.id, m.photo, to_char(b.writeDate, 'yyyy-mm-dd') writeDate, b.hashtag, b.fileName, b.hit "
					+ " from board b, member m ";
			// 검색어가 있는 경우 검색을 붙인다.
			sql += getCondition(pageObject);
			sql += " where (b.id = m.id) ORDER BY no DESC ";
			//    2) 순서 번호를 붙인다.
			sql = " select rownum rnum, no, title, content, id, photo, writeDate, hashtag, fileName, hit from(" + sql +") ";
			//    3) 1페이지에 해당되는 데이터 가져오기
			sql = " select rnum, no, title, content, id, photo, writeDate, hashtag, fileName, hit from (" + sql + ") "
...
//		System.out.println(totalRow);
	...
	public BoardVO view(long no, String id) throws Exception{
		// 리턴 타입과 동일한 변수 - 데이터가 있다면 데이터를 채워서 리턴시킨다.
		BoardVO vo = null;
		...
		try {
			...
			con = DB.getConnection();
			// 3. sql 작성 - 변경되는 데이터는 ? (대체문자)로 작성
			String sql = " SELECT b.no, b.title, b.content, m.id, "
					+ " to_char(b.writeDate, 'yyyy-mm-dd') writeDate, b.hashtag, b.fileName, "
					+ " m.photo, b.hit, "
					+ " (select 'LIKED' from board_like where no = ? and id = ? ) myLiked, "
					+ " (select count(*) from board_like where no = ? ) likeCnt "
					+ " from board b, member m "
					+ " WHERE (no = ?) and (b.id = m.id) ";
			System.out.println("board.view sql = " + sql);
			// 4. 실행 객체 & 데이터세팅 - no
			pstmt = con.prepareStatement(sql);
			//   데이터 타입에 따른 메서드를 선택해서 세팅해준다.
			pstmt.setLong(1, no);
	...
	public Integer update(BoardVO vo) throws Exception{
		...
		try {
			...
			con = DB.getConnection();
			// 3. SQL 작성
			String sql = "UPDATE board SET title = ?, content = ?, hashtag = ?, fileName = ? WHERE no = ?";
			// 4. 실행객체 & 데이터 세팅
			pstmt = con.prepareStatement(sql);
	...
	public Integer write(BoardVO vo) throws Exception{
		...
		try {
			...
			con = DB.getConnection();
			// 3. SQL 작성
			String sql = " INSERT INTO board(no, title, content, id, hashtag, fileName) VALUES (board_seq.NEXTVAL, ?, ?, ?, ?, ?) ";
	...
	// 번호 -> no 받아서 처리한다.
	// 데이터가 삭제가 됐으면 1이 안됐으면 0이 리턴된다.
	public Integer like(BoardVO vo) throws Exception{
		// return 타입과 동일한변수 선언
		...
		try {
			...
			con = DB.getConnection();
			// 3. SQL 작성
			String sql = "insert into board_like(no, id) values(?, ?)";
			// 4. 실행객체 & 데이터 세팅
			pstmt = con.prepareStatement(sql);
			...
			// 개발자를 위한 코드
			e.printStackTrace();
			throw new Exception("게시판 좋아요 - 좋아요 처리 중 DB 오류 또는 좋아요 처리 데이터입니다.");
		} finally {
				// 7. 닫기
				DB.close(con, pstmt);
		}
	...
	public Integer likeCancel(BoardVO vo) throws Exception{
		...
		try {
			...
			con = DB.getConnection();
			// 3. SQL 작성
			String sql = "delete from board_like where no = ? and id = ?";
			// 4. 실행객체 & 데이터 세팅
			pstmt = con.prepareStatement(sql);
			pstmt.setLong(1, vo.getNo());
			pstmt.setString(2, vo.getId());
			...
			// 개발자를 위한 코드
			e.printStackTrace();
			throw new Exception("게시판 좋아요 취소 - 좋아요 취소 중 DB 오류 또는 정보 확인하셔야 합니다.");
		} finally {
				// 7. 닫기
				DB.close(con, pstmt);
